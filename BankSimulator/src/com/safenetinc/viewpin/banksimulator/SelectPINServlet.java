 ...
 * @author Paul Hampton
 */
public class SelectPINServlet extends HttpServlet
{
    ...
    {
        getLogger().debug("SelectPINServlet v1 up and running");
...
*/
      ...
      //  PINList = user.getCardHolderDetails().getPinNumbers();
        Integer size = new Integer(PINList.size());
        getLogger().debug("the user has PINS: "+PINList.size());
        String pinListSize = new String(size.toString());
        request.getSession().setAttribute("pinList", PINList);
        request.getSession().setAttribute("pinListSize",pinListSize );
        ...
        try
        {
            getServletConfig().getServletContext().getRequestDispatcher("/SelectPIN.jsp").forward(request, response);
        }
        catch (Exception ioe)
        {
