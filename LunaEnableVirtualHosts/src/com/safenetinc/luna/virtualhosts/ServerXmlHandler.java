...
public class ServerXmlHandler
{
     ...
     */
    public static boolean deleteVirtualHost (String name) throws ServerXmlException
    {
        ...
        for (int i = 0; i < prop.size(); i++)
        {
			...
			{
				if (hostname.compareToIgnoreCase(name) == 0)
				{
					hostFound = true;
		...
		if(hostFound == false)
		{
			 System.out.println("No Virtual Host found with this name");
			 return false;
		}
        try
        {
        ...
        {
            System.out.println("Could not delete Virtual host");
			return false;
        }
    ...
    public static int getPortForVirtualHost (String name) throws ServerXmlException
    {
        ...
        for (int i = 0; i < prop.size(); i++)
        {
			...
			{
				System.out.println("Could not find path to this vitual host");
				return -1;
			}
     ...
     *         reason
     */
    public static void changeCertificateForConnector (int port, String certificatePath, String privateKeyPath) throws ServerXmlException
    {
        ...
        for (int i = 0; i < prop.size(); i++)
        {
            ...
            if (connectorPort == port)
            {
                ...
                xmlConfig.setProperty(CONFIG_PATH_TO_CONNECTOR + i + ")[@SSLCertificateFile]", certificatePath);
                xmlConfig.setProperty(CONFIG_PATH_TO_CONNECTOR + i + ")[@SSLCertificateKeyFile]", privateKeyPath);
                changed = true;
            }
        }
