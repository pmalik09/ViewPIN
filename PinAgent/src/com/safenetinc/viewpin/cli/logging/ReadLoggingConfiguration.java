	 ...
	 public static boolean checkFileStatus(String virtualHostName)
 	 {
    	...
    	configFile = Constants.Webapp_Directory +virtualHostName+Constants.Config_Directory+ Constants.Logging_Config_File;
    
    	boolean fileStatus = FileHandler.fileExists(Constants.Logging_Config_File, new File(Constants.Webapp_Directory +virtualHostName+Constants.Config_Directory));
			...
			if(0 != element.compareToIgnoreCase(Constants.ELEMENT_CATEGORY))
			{
				...
				try
				{
					for(int i=0;i<nodes.getLength();i++)
					{
					   ...
					   if((valueName != null)&& (valueName.trim().compareToIgnoreCase(elementName) == 0))
					   {
					...
					if(paramlist != null && paramlist.getLength() > 0)
					{
					   ...
					   for(int j = 0; j< paramlist.getLength(); j++)
					   {
						...
						if((paramName != null)&& (paramName.trim().compareToIgnoreCase(parameterName) == 0))
						{
